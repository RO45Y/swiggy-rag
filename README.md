...
3. **Retrieval** (`src/retriever.py`):  
   ...
   - **Cross‑encoder reranker** (`cross-encoder/ms-marco-MiniLM-L-12-v2`) to re‑order the best candidates.
...
4. **Install system dependencies for OCR (macOS)**
   ```bash
...
6. **Place the Swiggy Annual Report**
...
1. **Create a test set** – `eval_questions.json` should contain questions and expected page numbers, e.g.:
